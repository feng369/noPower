...
//        try {
//            Claims claims = Jwts.parser().setSigningKey(getKey(appId)).parseClaimsJws(token).getBody();
//            log.info("appId="+appId+":token="+token+":"+":ID="+claims.getId()+":Subject="+claims.getSubject()+":Issuer="+claims.getIssuer()+":Expiration="+claims.getExpiration());
//            return claims.getSubject().equals(appId);
//        } catch (Exception e) {
////            e.printStackTrace();
//            log.error("appId="+appId+":token="+token+"::"+e.getMessage());
//            return false;
//        }
        return true;
    }
