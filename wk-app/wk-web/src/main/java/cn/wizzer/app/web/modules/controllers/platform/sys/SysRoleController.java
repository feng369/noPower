...
//            map.put("isUnit","");
    ...
    public Object addDo(@Param("menuIds") String menuIds, @Param("..") Sys_role role, HttpServletRequest req) {
        try {
            ...
            if ("root".equals(role.getUnitid()))
                role.setUnitid("");
            if(role.getIsDept()==0&&role.getIsUnit()==0){
                role.setIsSelf(1);
            }
    ...
    @At("/editData/?")
    @Ok("beetl:/platform/sys/role/editData.html")
    @RequiresPermissions("sys.manager.role")
    public void editData(String id, HttpServletRequest req) {
        req.setAttribute("obj", roleService.fetch(id));
    }

    @At
    @Ok("json")
    @RequiresPermissions("sys.manager.role")
    @SLog(tag = "修改数据权限", msg = "${args[0].id}")
    public Object editDataDo(@Param("..") Sys_role role, HttpServletRequest req) {
        try {
            //update
    ...
    @Ok("json:full")
    @RequiresPermissions("sys.manager.role")
    public Object selectData(@Param("disabled")String disabed,@Param("roleid") String roleid, @Param("name") String name, @Param("length") int length, @Param("start") int start, @Param("draw") int draw, @Param("::order") List<DataTableOrder> order, @Param("::columns") List<DataTableColumn> columns) {
        ...
        if(!Strings.isBlank(disabed)){
            sql += " and a.disabled = "+ disabed  + " ";
        }
        String s = sql;
        if (order != null && order.size() > 0) {
            for (DataTableOrder o : order) {
    ...
    public Object delUser(@Param("menuIds") String menuIds, @Param("roleid") String roleid, HttpServletRequest req) {
        try {
            String[] ids = StringUtils.split(menuIds, ",");
            roleService.clear("sys_user_role", Cnd.where("userId", "in", ids).and("roleId", "=", roleid));
            Sys_role role = roleService.fetch(roleid);
            req.setAttribute("name", role.getName());
            return Result.success("system.success");
        } catch (Exception e) {
    ...
    public Object pushUser(@Param("menuIds") String menuIds, @Param("roleid") String roleid, HttpServletRequest req) {
        try {
            ...
            for (String s : ids) {
                if (!Strings.isEmpty(s)) {
                    int cn = roleService.count("sys_user_role",Cnd.where("roleId","=",roleid).and("userId","=",s));
                    if(cn<=0) {
