...
public class SysLoginController {
    ...
    public Object doLogin(@Attr("loginToken") AuthenticationToken token, HttpServletRequest req, HttpSession session) {
        ...
        try {
            ...
            subject.login(token);
            Sys_user user = (Sys_user) subject.getPrincipal();
            base_person basePerson=basePersonService.getPersonInfo();
            String airportid= basePerson.getAirportid();
            String customerid=basePerson.getCustomerId();
            if(!Strings.isBlank(airportid)){
                base_airport baseAirport = baseAirportService.fetch(airportid);
                subject.getSession().setAttribute("airportid", baseAirport.getId());
                subject.getSession().setAttribute("airportname", baseAirport.getAirportname());
            }
            if(!Strings.isBlank(customerid)){
                base_customer baseCustomer = baseCustomerService.fetch(customerid);
                subject.getSession().setAttribute("customerid", baseCustomer.getId());
                subject.getSession().setAttribute("customername", baseCustomer.getCustomername());
            }
            subject.getSession().setAttribute("personid", basePerson.getId());
            ...
            //return "jCallback("+Result.success("login.success").toString()+")";
            //return "jCallback"+"({\"flag\":\"successfuly\"})";
            //return jCallback+"({\"a\":0})";
    ...
    @Ok("json")
    @Filters(@By(type = PlatformAuthenticationFilter.class))
    public Object doTokenLogin(@Attr("loginToken") AuthenticationToken token, HttpServletRequest req, HttpSession session) {
        int errCount = 0;
        try {
